...
function getGameCreater(io){
	...
	gameCreater.createWall = function(x,y,width,height,imgsrc,isBlock,restitution){
		var wallBody;
		var fixDef = this.createFixture(new b2PolygonShape,width,height);
		if(typeof restitution != 'undefined') fixDef.restitution = restitution;
		...
		wallBody.name = 'wall';
		wallBody.width = width;	//box2d对象获取width，height困难
		wallBody.height = height;
		if(imgsrc){
			this.renderShape(wallBody,fixDef).addImage(imgsrc);
		}else{
	...
	gameCreater.createBox = function(x,y,width,height,imgsrc,restitution){
		var boxBody;
		var fixDef = this.createFixture(new b2PolygonShape,width,height);
		...
		boxBody.width = width;	//box2d对象获取width，height困难
		boxBody.height = height;

		if(imgsrc){
			this.renderShape(boxBody,fixDef).addImage(imgsrc);
		}else{
			this.renderShape(boxBody,fixDef).setFillStyle(Default.fill).setStrokeStyle(Default.stroke);
		}
		return boxBody;

	}

	gameCreater.createSpring = function(x,y,width,height,imgsrc,restitution){
		var body;
		var fixDef = this.createFixture(new b2PolygonShape,width,height);
		fixDef.restitution = restitution || .6;
		var bodyDef = this.createBody(x,y);
		bodyDef.type =  b2Body.b2_staticBody;
		...
		body.width = width;	//box2d对象获取width，height困难
		body.height = height;

		if(imgsrc){
			this.renderShape(body,fixDef).addImage(imgsrc);
		}else{
			this.renderShape(body,fixDef).setFillStyle('yellow').setStrokeStyle(Default.stroke);
		}
		return body;

	}

	gameCreater.createTimeGate = function(x,y,width,height,fromNum,toNum,imgsrc){
		var body;
		var fixDef = this.createFixture(new b2PolygonShape,width,height);
		var bodyDef = this.createBody(x,y);
		bodyDef.type =  b2Body.b2_staticBody;
